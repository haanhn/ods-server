...
const url =
  'http://apilayer.net/api/live?access_key=795ffe55cd64ba290653c2cb689bf056&currencies=vnd&source=USD&format=1';
...
exports.closeCampaign = async (campaign) => {
  console.log('============Close campaign function');
  let amplitudeFluctuations = campaign.campaignGoal * 0.03 > 10000000 ? 10000000 : campaign.campaignGoal * 0.03;
  console.log('============Cbiên độ giao động:', amplitudeFluctuations);

  if (campaign.autoClose) {
    const raise = await campaignService.getRaise(campaign.id);
    console.log('============raise:', campaign.campaignGoal - amplitudeFluctuations);
    if (raise >= campaign.campaignGoal - amplitudeFluctuations && campaign.campaignStatus !== 'close') {
...
const create_payment_json = async (req) => {
  ...
  return (payment_json = {
    ...
    redirect_urls: {
      return_url: 'http://127.0.0.1:5000/api/donations/paypal/success?amount=' +
        req.body.amount +
        '&userId=' +
        userId +
        '&campaignId=' +
        ...
        '&email=' +
        email,
      cancel_url: 'http://127.0.0.1:5000/api/donations/paypal/cancel',
    },
    transactions: [{
      item_list: {
        items: [{
